import type { builtinPlugins } from "svgo";

export type PluginName = Exclude<
	(typeof builtinPlugins)[number]["name"],
	"default-preset"
>;

export const descriptions: Partial<Record<PluginName, string>> = {
	removeDoctype:
	...
	removeXMLProcInst: `Removes the [XML declaration](https://developer.mozilla.org/docs/Web/XML/XML_introduction#xml_declaration) from the document.
...
It can be safely removed without impacting compatibility with SVG clients. However, some tools may fail to detect the MIME-type as \`image/svg+xml\` if this is removed.`,
	removeComments: `Removes XML comments from the document.
...
Removing a comment like this may be considered a breach of the license terms, as Font Awesome Free is released under [CC-BY-4.0 (Creative Commons Attribution)](https://creativecommons.org/licenses/by/4.0/), but removing the comment would strip away that attribution.`,
	removeMetadata: `Removes the [\`<metadata>\`](https://developer.mozilla.org/docs/Web/SVG/Element/metadata) element from the document.
...
There may be cases you'd want to disable this plugin, as some SVGs include copyright and licensing information in the metadata. In particular, documents that reference the [Creative Commons](http://creativecommons.org/ns#) namespace.`,
	removeEditorsNSData: `Removes all XML [namespaces](https://developer.mozilla.org/docs/Web/SVG/Namespaces_Crash_Course), elements, and attributes associated with popular vector editing software.
...
You can find a list of removed editor namespaces in [\`_collections.js\`](https://github.com/search?q=repo%3Asvg%2Fsvgo+editorNamespaces&type=code), which includes software like [Inkscape](https://inkscape.org/), Adobe Illustrator, and Figma.`,
	cleanupAttrs: `Removes redundant whitespaces from attribute values.
...
This will not modify the attribute keys, nor remove them if the value becomes empty after optimization.`,
	...
	cleanupIds: `Removes unused IDs, and minifies IDs that are referenced by other elements.
...
By default, we back off from removing and minifying IDs if a \`<script>\` or \`<style>\` element is present in the document. You can bypass this behavior by setting the \`force\` parameter to \`true\`.`,
	...
	convertColors: `Converts color references to the shortest equivalent.
...
It makes no difference which format is received by a client, and each one has wide support across browsers and image viewing software.`,
	removeUnknownsAndDefaults: `Removes unknown elements and attributes, as well as attributes that are set to their default value.
...
Assigning these attributes can sometimes change nothing in the document. For example, in most cases assigning a \`stroke\` color is redundant if the elements [\`stroke-width\`](https://developer.mozilla.org/docs/Web/SVG/Attribute/stroke-width) or [\`stroke-opacity\`](https://developer.mozilla.org/docs/Web/SVG/Attribute/stroke-opacity) is \`0\`.`,
	cleanupEnableBackground: `Cleans up [\`enable-background\`](https://developer.mozilla.org/docs/Web/SVG/Attribute/enable-background), unless the document uses [\`<filter>\`](https://developer.mozilla.org/docs/Web/SVG/Element/filter) elements.
...
- Replace the value with \`new\` when the width and height values match the width and height of a \`<mask>\` or \`<pattern>\` node.`,
	removeHiddenElems: `Remove hidden or invisible elements from the document. This can be elements with 0 width and height defined, or elements that were just hidden with CSS.
...
Refer to the parameters for the conditions this plugin looks for. All checks enabled by default.`,
	...
	moveGroupAttrsToElems: "Move a groups attributes to their child elements.",
	collapseGroups: `Finds groups that effectively do nothing and flattens them, preserving the contents of the groups.
...
Groups can be formed using the [\`<g>\`](https://developer.mozilla.org/docs/Web/SVG/Element/g) element. They're used for organizing the document, or applying [presentation attributes](https://developer.mozilla.org/docs/Web/SVG/Attribute/Presentation) to all children contained in a group.`,
	convertPathData: `Optimize path commands found in \`<path>\`, \`<glyph>\`, and \`<missing-glyph>\` elements. Path commands are the syntax used in the \`d\` attribute, each character represents an instruction to draw paths.
...
You can read more about the plugins capabilities by going through the individual parameters.`,
	...
	removeEmptyAttrs: `Remove empty attributes from elements in the document.
...
- \`requiredExtensions\`
- \`systemLanguage\``,
	removeEmptyContainers: `Remove container elements in the document that have no children or meaningful attributes, excluding the \`<svg>\` element which is ignored.
...
- [\`<symbol>\`](https://developer.mozilla.org/docs/Web/SVG/Element/symbol)`,
	removeUnusedNS: "Removes unused namespace declarations from the document.",
	mergePaths: "Merge multiple paths into one.",
	sortAttrs: `Sorts attributes in all elements in the document. This does not reduce the size of the SVG, but improves readability and _may_ improve how compression algorithms perform on it.
...
² Uses the default plugins preset as-is.`,
	sortDefsChildren: `Sorts all children in the \`<defs>\` element. This does not reduce the size of the SVG, but _may_ improve how compression algorithms perform on it.
...
¹ Uses the default plugins preset excluding \`sortAttr\` and \`sortDefsChildren\`.
² Uses the default plugins preset as-is.`,
	removeDesc: `Removes the [\`<desc>\`](https://developer.mozilla.org/docs/Web/SVG/Element/desc) element from the document in one of the following conditions:
